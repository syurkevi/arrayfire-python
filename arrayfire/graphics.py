...
from .defaults import COLORMAP, MARKER, c_bool_t, c_char_ptr_t, c_double_t, c_float_t, c_int_t, c_pointer, c_void_ptr_t
...
class Window:
    ...
    def __init__(self, width=1280, height=720, title="ArrayFire"):
        ...
        safe_call(backend.get().af_create_window(
            c_pointer(self._wnd), c_int_t(_width), c_int_t(_height), c_char_ptr_t(_title)))
    ...
    def scatter(self, X, Y, Z=None, points=None, marker=MARKER.POINT, title=None):
        ...
        if points is None:
            if Z is None:
                safe_call(
                    backend.get().af_draw_scatter_2d(self._wnd, X.arr, Y.arr, marker.value, c_pointer(_cell)))
            else:
                safe_call(
                    backend.get().af_draw_scatter_3d(self._wnd, X.arr, Y.arr, Z.arr, marker.value, c_pointer(_cell)))
        else:
    ...
    def scatter2(self, points, marker=MARKER.POINT, title=None):
        ...
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().af_draw_scatter2(self._wnd, points.arr, marker.value, c_pointer(_cell)))
    ...
    def scatter3(self, points, marker=MARKER.POINT, title=None):
        ...
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().af_draw_scatter3(self._wnd, points.arr, marker.value, c_pointer(_cell)))
    ...
    def vector_field(self, xpoints, xdirs, ypoints, ydirs, zpoints=None, zdirs=None,
        ...
        """
        _cell = _Cell(self._r, self._c, title, self._cmap)
        # BUG: line and Z variables are not defined.
        if line is None:
            if Z is None:
                safe_call(backend.get().af_draw_vector_field_2d(
                    self._wnd, xpoints.arr, ypoints.arr, xdirs.arr, ydirs.arr, c_pointer(_cell)))
            else:
                safe_call(backend.get().af_draw_vector_field_2d(
    ...
    def hist(self, X, min_val, max_val, title=None):
        ...
        """
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().af_draw_hist(
            self._wnd, X.arr, c_double_t(max_val), c_double_t(min_val), c_pointer(_cell)))
    ...
    def set_axes_limits(self, xmin, xmax, ymin, ymax, zmin=None, zmax=None, exact=False):
        ...
        if zmin is None or zmax is None:
            safe_call(backend.get().af_set_axes_limits_2d(
                self._wnd, c_float_t(xmin), c_float_t(xmax), c_float_t(ymin), c_float_t(ymax), exact,
                c_pointer(_cell)))
        else:
            safe_call(backend.get().af_set_axes_limits_2d(
                self._wnd, c_float_t(xmin), c_float_t(xmax), c_float_t(ymin), c_float_t(ymax), c_float_t(zmin),
                c_float_t(zmax), exact, c_pointer(_cell)))
