...
def stdev(a, bias=VARIANCE.DEFAULT, dim=None):
    ...
    if dim is not None:
        out = Array()
        safe_call(backend.get().af_stdev_v2(c_pointer(out.arr), a.arr, bias.value, c_int_t(dim)))
        return out

    real = c_double_t(0)
    imag = c_double_t(0)
    safe_call(backend.get().af_stdev_all(c_pointer(real), c_pointer(imag), a.arr, bias.value))
    real = real.value
    imag = imag.value
    return real if imag == 0 else real + imag * 1j


def cov(a, bias=VARIANCE.DEFAULT, dim=None):
    """
    ...
    if dim is not None:
        out = Array()
        safe_call(backend.get().af_cov_v2(c_pointer(out.arr), a.arr, bias.value, c_int_t(dim)))
        return out

    real = c_double_t(0)
    imag = c_double_t(0)
    safe_call(backend.get().af_cov_all_v2(c_pointer(real), c_pointer(imag), a.arr, bias.value))
    real = real.value
    imag = imag.value
    return real if imag == 0 else real + imag * 1j
